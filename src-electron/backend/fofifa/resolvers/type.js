...
module.exports = {
  DESCENTE: {
    ...
    saisisseurs: async (_, args, context) => {
      let res = await context.database.query(
        `SELECT *, 'SAISISSEUR' AS groupe  FROM affecter
  ...
  MISSION: {
    lieu: async (_, args, context) => {
      const res = await model.lieu.findOne({
        raw: true,
        where: { IdLieu: _.IdLieu },
        attributes: [
          ['descriLieu', 'district'],
          ['IdLieu', 'IdDistrict'],
          'IdLieu',
    ...
    descente: async (_, context, resolveInfo) => {
      return await model.descente.findOne({
        raw: true,
        where: { IdDescente: _.IdDescente }
      });
    },
    equipe: async (_, args, context) => {
      const res = await context.database.query(
        'SELECT * FROM mission INNER JOIN charger ON charger.IdMission=mission.IdMission INNER JOIN enqueteur ON enqueteur.IdPersonne=charger.IdPersonne WHERE mission.IdMission=:idm',
        {
          replacements: { idm: _.IdMission },
          type: seq.QueryTypes.SELECT
        }
      );
      return res;
    ...
    districts: async (_, args, context) => {
      ...
      if (_.IdDescente) {
        ...
        if (_.IdDisctrictOfMission) {
          ...
          query =
            `SELECT IdLieu as IdDistrict, descriLieu as district , IdRegion
                        FROM lieu as li WHERE li.IdLieu ` +
            condition +
            `(SELECT dist.IdLieu FROM descente as dst
                        INNER JOIN mission as miss ON miss.IdDescente= dst.IdDescente
                        INNER JOIN lieu as dist ON dist.IdLieu = miss.IdLieu
                        INNER JOIN lieu as reg ON reg.IdLieu = dist.IdRegion
  ...
  ENQUETEUR: {
    ...
    missions: async (_, args, context) => {
      const res = await context.database.query(
        'SELECT * FROM mission INNER JOIN charger ON charger.IdMission=mission.IdMission INNER JOIN enqueteur ON enqueteur.IdPersonne=charger.IdPersonne WHERE enqueteur.IdPersonne=:idp',
        {
          replacements: { idp: _.IdPersonne },
          type: seq.QueryTypes.SELECT
        }
      );
  ...
  EA: {
    ...
    moes: async (_, args, context) => {
      ...
      const res = await model.moe.findAll({
        raw: true,
        where: {
          IdEA: _.IdEA
        }
  ...
  MENAGE: {
    activitePricipale: async (_, args, context) => {
      const res = await model.param_divers.findOne({
        where: {
          IdParam: _.IdActPcpl,
    ...
    assiciations: async (_, args, context) => {
      const res = await model.etre_membre.findAll({
        raw: true,
        where: {
          IdPersonne: _.IdPersonne
        }
  ...
  MOE: {
    ...
    avantegeNature: async (_, args, context) => {
      const res = await model.avantage_nat.findAll({
        raw: true,
        where: {
          IdPersonne: _.IdPersonne
        }
