...
module.exports = {
  Query: {
    login: async (_, args, context) => {
      ...
      if (!usr) {
        ...
        const idUser = usr_copy.IdPersonne;
      ...
      if (!valid) {
        ...
        res.token = jwt.sign(
          {
            IdPersonne: usr_copy.IdPersonne,
            usenrame: usr_copy.username,
            groupe: groupe
          },
          sing_scret_key,
          {
    ...
    checkConnexion: async (_, args, context) => {
      ...
      const res = {
        token: auth.token,
        groupe: auth.userInfo.groupe,
        expiration: expiration_login,
        user: auth.userInfo,
        IdPersonne: auth.userInfo.IdUser
      };
    ...
    users: async (_, args, context) => {
      ...
      const chercheurs = await context.database.query(
        'SELECT *, "CHERCHEUR" AS groupe FROM chercheur as us INNER JOIN fofifapers as ffp ON ffp.IdPersonne=us.IdPersonne',
        {
          type: seq.QueryTypes.SELECT
        }
      );

      const enqueteur = await context.database.query(
        'SELECT *, "ENQUETEUR" AS groupe FROM enqueteur as us INNER JOIN fofifapers as ffp ON ffp.IdPersonne=us.IdPersonne',
        {
          type: seq.QueryTypes.SELECT
        }
      );
      const saisisseur = await context.database.query(
        'SELECT *, "SAISISSEUR" AS groupe FROM saisisseur as us INNER JOIN fofifapers as ffp ON ffp.IdPersonne=us.IdPersonne',
        {
          type: seq.QueryTypes.SELECT
        }
      );

      const res = [...chercheurs, ...enqueteur, ...saisisseur];
    ...
    user: async (_, args, context) => {
      let res = await context.database.query(
        'SELECT *, "' +
          args.groupe +
          '" AS groupe FROM ' +
          args.groupe.toLowerCase() +
          ' as us INNER JOIN fofifapers as ffp ON ffp.IdPersonne=us.IdPersonne WHERE us.IdPersonne = :idp',
        {
          replacements: { idp: args.IdUser },
          type: seq.QueryTypes.SELECT
        }
      );
    ...
    availableEnqueteurForDescente: async (_, args, context) => {
      ...
      const res = await context.database.query(
        `SELECT * FROM enqueteur as enq INNER JOIN fofifapers as ffp ON ffp.IdPersonne = enq.IdPersonne WHERE enq.IdPersonne NOT IN  (SELECT enq.IdPersonne FROM descente as des
        ...
        {
          replacements: { idd: args.IdDescente, idl: args.IdDistrict },
          type: seq.QueryTypes.SELECT
        }
      );
  ...
  Mutation: {
    newUser: async (_, args, context) => {
      ...
      const addedBase = await model.personne.create({
        nom: args.userInfo.nom,
        prenom: args.userInfo.prenom,
        sexe: args.userInfo.sexe,
        age: args.userInfo.age
      });

      const salt = keyGen.randomBytes(32).toString('hex');
      ...
      let added = await model.fofifapers.create({
        IdPersonne: addedBase.IdPersonne,
        username: args.loginInfo.username,
        password: crypto.encrypt(args.loginInfo.password),
        salt: salt
      });
      const usrAdded = added;

      if (args.groupe == 'CHERCHEUR') {
        added = await model.chercheur.create({
          IdPersonne: addedBase.IdPersonne
        });
      } else if (args.groupe == 'ENQUETEUR') {
        added = await model.enqueteur.create({
          IdPersonne: addedBase.IdPersonne
        });
      } else if (args.groupe == 'SAISISSEUR') {
        added = await model.saisisseur.create({
          IdPersonne: addedBase.IdPersonne
        });
      }
      ...
      return {
        ...
        ...{
          ...usrAdded.dataValues,
          actif: parseInt(usrAdded.dataValues.actif)
        },
        ...{
          ...addedBase.dataValues,
          sexe: parseInt(addedBase.dataValues.sexe)
        }
      };
    },
    ...
    deleteUser: async (_, args, context) => {
      if (args.groupe == 'ENQUETEUR') {
        const is_ocuped = model.charger.findAll({
          limit: 1,
          raw: true,
          where: {
            IdPersonne: args.IdUser
