...
module.exports = {
  Query: {
    associations: async (_, args, context) => {
      ...
      const res = await model.association.findAll({
        raw: true
      });
    ...
    association: async (_, args, context) => {
      ...
      const res = await model.association.findOne({
        raw: true,
        where: {
    ...
    regions: async (_, args, context) => {
      ...
      const res = await model.lieu.findAll({
        raw: true,
        where: {
          IdRegion: {
            [Op.is]: null
    ...
    parametre: async (_, args, context) => {
      ...
      if (
        context.req.auth.userInfo.groupe != 'CHERCHEUR' &&
        context.req.auth.userInfo.groupe != 'ENQUETEUR'
      ) {
        throw new Error(msg.notAllowedApi);
      }
  ...
  Mutation: {
    addAssociation: async (_, args, context) => {
      ...
      const assoc = await model.association.create(
        { nomAssoc: args.nomAssoc, IdType: args.IdTypeAssoc },
        {
          raw: true
        }
      );
    ...
    updateAssociation: async (_, args, context) => {
      ...
      await model.association.update(
        { nomAssoc: args.nomAssoc, IdType: args.IdTypeAssoc },
        {
          raw: true,
          where: {
    ...
    addRegion: async (_, args, context) => {
      const added = await model.lieu.create({ descriLieu: args.region });
      // console.log(added);
    ...
    updateRegion: async (_, args, context) => {
      ...
      await model.lieu.update(
        { descriLieu: args.region },
        {
          where: {
            IdLieu: args.IdRegion,
            IdRegion: {
    ...
    addDistrict: async (_, args, context) => {
      const added = await model.lieu.create({
        descriLieu: args.district,
        IdRegion: args.IdRegion
      });
      console.log(added);
    ...
    addParam: async (_, args, context) => {
      ...
      const added = await model.param_divers.create({
        tableParam: args.table,
        codeParam: args.code,
        val_param: args.val,
      ...
      return {
        ...{ ...added.dataValues, status: added.dataValues.status },
        ...args
      };
    },
    ...
    updateParam: async (_, args, context) => {
      ...
      const res = await model.param_divers.findByPk(args.IdParam, {
        raw: true,
        attributes: [
          'IdParam',
          ['tableParam', 'table'],
