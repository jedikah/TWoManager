...
module.exports = {
  ...
  Mutation: {
    addEA: async (_, args, context) => {
      ...
      const charger = await model.charger.findOne({
        raw: true,
        where: {
      ...
      const status = await model.param_divers.findOne({
        where: {
          codeParam: 'EN_SAISI'
        },
        raw: true
      ...
      const added = await model.EA.create({
        codeEA: args.codeEA,
    ...
    updateEA: async (_, args, context) => {
      ...
      const res = await model.EA.findOne({
        raw: true,
        where: {
          IdEA: args.IdEA
        }
