...
module.exports = {
  Query: {
    descentes: async (_, args, context) => {
      ...
      const res = await model.descente.findAll();
    ...
    descente: async (_, args, context) => {
      ...
      //     throw  new Error(msg.notConnectedUser);
      // }
      const res = await model.descente.findByPk(args.IdDescente);
  ...
  Mutation: {
    addDescente: async (_, args, context) => {
      ...
      const added = await model.descente.create({
        description: args.description,
        dateDescente: args.dateDescente
      });
      return added;
    },
    ...
    updateDescente: async (_, args, context) => {
      ...
      await model.descente.update(
        { description: args.description, dateDescente: args.dateDescente },
        {
          where: {
